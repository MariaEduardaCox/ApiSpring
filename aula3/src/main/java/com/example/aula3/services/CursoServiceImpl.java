...
public class CursoServiceImpl implements CursoService{
    ...
    public Curso salvar(CursoDTO cursoDTO) {
        CategoriaCurso categ = categoriaCursoRepository.findById(cursoDTO.getCategoriaCursoId()).orElseThrow(()-> new RegraNegocioException("Código da categoria não encontrado!"));
    ...
    public List<CursoDTO> listarTodos(){
        ...
        for(Curso c : cursos){
            cursosDTO.add(new CursoDTO(c.getId(), c.getNome(), c.getCargaHoraria(), c.getCategoriaCurso() != null ? c.getCategoriaCurso().getId(): 0));
        }
        return cursosDTO;
    }
